...
This is where Flume comes in. Flume provides a visual editor that lets Sundown Plants model the logic for calculating their pricing. As the developer you just create the nodes that output the time of the year, and which items are in the cart, and your customer can take care of the rest. If this process is too complicated for a particular customer, you use Flume as your own internal tool rather than hardcoding logic for different customers. Your code doesn't need to change, and you don't need to deploy a new build every time the logic needs of a customer changes. The JSON graphs representing each customers logic can be stored in a database, and pulled on demand when your app loads, ensuring that each customer is only running the logic necessary for their business.
...
There's a lot more to graph theory than we've discussed here. If you're interesting in learning more the [Wikipedia article](https://en.wikipedia.org/wiki/Graph_theory) on the subject is a good place to start.
