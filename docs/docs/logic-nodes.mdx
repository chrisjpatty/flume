...
import { SimpleEditor4, SimpleEditor5 } from '../components/GettingStartedExample'
...
Let's pick a few ideas from the list above. We'll first create a node that represents the current user, and then a node that concatenates strings. We already have all the ports we need, so let's add a new user node in `config.js`:
