...
Allowing users to define custom logic can seem like a scary proposition at first.

_"How will my users know to setup their logic in a way that won't crash?"_

_"How will my users know what nodes are compatible with each other?"_
...
> As a note, while familiarity with the concept of type safety may be helpful for developing with Flume, Typescript itself is *not* required (although it is supported).
...
<br/>
<FlexRow center>
  <img src="/public/img/valid_port_types.png" alt="A flume grid showing a true/false node and a text node connected successfully to a text switch node." style={{maxWidth: 800, width: '100%'}} />
</FlexRow>
<br/>
